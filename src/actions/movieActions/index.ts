...
export const fetchMovies = (): ThunkResult<void> => async dispatch => {
  ...
  try {
    const response: AxiosResponse<Movies> = await api.get('/movies');
    dispatch({ type: MoviesActions.FETCH_MOVIES_SUCCESS, payload: response.data });
  } catch (e) {
    dispatch({ type: MoviesActions.FETCH_MOVIES_FAIL });
  }
};

export const fetchMovie = (id: number): ThunkResult<void> => async dispatch => {
  dispatch({ type: MoviesActions.FETCH_MOVIE });
  try {
    const response: AxiosResponse<Movie> = await api.get(`/movies/${id}`);
    dispatch({ type: MoviesActions.FETCH_MOVIE_SUCCESS, payload: response.data });
  } catch (e) {
    dispatch({ type: MoviesActions.FETCH_MOVIE_FAIL });
  }
};

export const editMovie = (editedMovie: Movie): ThunkResult<void> => async dispatch => {
...
export const deleteMovie = (deletedId: number): ThunkResult<void> => async dispatch => {
