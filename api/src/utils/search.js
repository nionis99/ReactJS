const searchMovies = (movies, query) => {
  ...
  if (search && searchBy) {
    switch (searchBy) {
      case 'title':
        data = movies.filter((item) => new RegExp(search, 'i').test(item.title));
        break;

      case 'genres': {
        const query = `${search[0].toUpperCase()}${search.substr(1)}`;
        data = movies.filter((item) => item.genres.some((x) => x.includes(query)));
        break;
      }
    }
  }
...
const filterMovies = (movies, query) => {
  ...
  if (parsedGenres && parsedGenres.length) {
    ...
    data = movies.filter((item) => {
      ...
      const count = array.reduce((prev, curr) => prev + (filterMap[curr.toLowerCase().trim()] ? 1 : 0), 0);
